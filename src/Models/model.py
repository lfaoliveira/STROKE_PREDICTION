...
class MLP(LightningModule):
    ...
    def validation_step(self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx: int):
        ...
        """ print(
            f"Current experiment: {mlflow.get_experiment(mlflow.active_run().info.experiment_id).name if mlflow.active_run() else 'No active run'}"
        ) """
        ...
        # print(f"RUN LOGGER: {self.logger}")
        try:
